...
if ssh ${ssh_remote} test -d ${new_release_dir}; then
  printf "ERROR: ${new_release_dir} already exists. Skipping...\n"
  exit 1
fi

create_next_release_repo() {
  #create dir locally
...
<repository name='Eclipse SimRel ${release_name}'  type='org.eclipse.equinox.internal.p2.artifact.repository.CompositeArtifactRepository' version='1.0.0'>
  <properties size='3'>
...
<repository name='Eclipse SimRel ${release_name}'  type='org.eclipse.equinox.internal.p2.metadata.repository.CompositeMetadataRepository' version='1.0.0'>
  <properties size='3'>
    <property name='p2.timestamp' value='${timestamp}'/>
    <property name='p2.compressed' value='true'/>
    <property name='p2.atomic.composite.loading' value='true'/>
